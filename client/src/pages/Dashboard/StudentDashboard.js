...
import ThesisSubmitted from "../../components/Dashboard/ThesisSubmitted";
...
function StudentDashboard() {
  ...
  useEffect(() => {
    ...
    const getThesisCount = async() => {
      await axios
        .get(`http://localhost:3001/getThesisCountByUserId/${userData.user_id}`)
        .then(function (response) {
  ...
  const tabs = ['Profile', 'Thesis Submitted']
  const [currTab, setCurrTab] = useState(0)

  const tabView = () => {
    switch (currTab) {
      case 0: return (<Profile />)
      case 1: return <ThesisSubmitted />
    ...
    <>
      <div className="bg-white  flex flex-col items-center w-full">
        <div className="w-full shadow mx-auto py-6 px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-3xl font-bold text-gray-900"> Researchers Dashboard</h1>
        </div>
