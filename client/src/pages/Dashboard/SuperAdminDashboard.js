...
function SuperAdminDashboard() {
  ...
  useEffect(() => {
    ...
    await axios
      ...
      .then(function (response) {
        setCollegeCount(response.data);
        console.log("College Count: ", response.data)
      })
      .catch(function (error) {
        console.log("Error fetching College College ", error);
      })
    }
    ...
    const getThesisCount = async() => {
      await axios
        ...
        .then(function (response) {
          ...
          console.log("Thesis Count: ", response.data)

        })
        .catch(function (error) {
          console.log("Error fetching Total Thesis ", error);
        })
      }
      const getTodayThesisCount = async() => {
        await axios
          .get(`http://localhost:3001/getTodayNoOfThesis`)
          .then(function (response) {
            setTodayThesisCount(response.data);
            console.log("Today Thesis Count : ", response.data)
            
          })
          .catch(function (error) {
            console.log("Error fetching Todays Total Thesis ", error);
          })
        }
    getCollegeCount()
    getThesisCount()
  ...
  const tabs = ['Profile', 'College Requests', 'Researchers Requests', 'Researchers']
  const [currTab, setCurrTab] = useState(0)

  const tabView = () => {
    switch (currTab) {
      case 0: return (<Profile />)
      case 1: return <CollegeReqs />
      case 2: return <ResearchersReqs />
